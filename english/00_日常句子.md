...
- 因为性能损失，所以希望。。
  The extra hop from the index to the heap file **is too much of a performance penalty** for reads, so **it can be desirable to** store the indexed row directly within an index.
...
- 臭名昭著
  Java serialization **is notorious for** its bad performance and bloated encoding.
- 尽管有问题，但并未消亡
  **Despite all these problems**, RPC isn't going away.
- 丧失某种优点
  **Negate** the benefit of read scaling.
