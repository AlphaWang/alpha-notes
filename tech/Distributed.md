...
- **Durability 持久性**：一旦事务生效，其修改内容就不会丢失
  - 实现：
...
- **脏写 / Dirty Writes**：覆盖了另一事务尚未提交的写入。——所有隔离级别均能解决脏写
  ![dirty-write](../img/distributed/txn-dirty-write.png)
...
- 选型
  ...
  - 考虑问题：如果系统形成 A\B 两个网络分区，分别只能从各自区域内的服务发现节点获取服务坐标，是否有影响？
  - 无影响则用 AP。
...
- IO多路复用技术
  - 支持IO多路复用的系统调用：select, poll, epoll (Java NIO 基于epoll机制)
  
    ![image-20230716141634721](../img/distributed/msg-io-epoll.png)
  
- Reactor 模型

  - 当主流程收到请求后，通过多路分离处理的方式，把请求分发给相应的请求处理器处理。

  - **Reactor**：负责监听和分配事件。收到事件后分派给对应的 Handler 处理，事件包括连接建立就绪、读就绪、写就绪等。

    > 还可升级为主从Reactor：主线程只负责接收新连接，子线程负责完成后续的业务处理
  ...
  - ![image-20230716141846117](../img/distributed/msg-io-reactor.png)
...
- **元数据的存储**
  - 要求：强一致、高可靠 不允许丢失；要通知到所有Broker节点。
  - 思路：
    - 基于第三方组件：ZK，NameServer。开发成本低，但引入运维复杂度。
    - 或在集群内部实现RAFT：例如Kafka去zk版本（[KIP-500](https://cwiki.apache.org/confluence/display/KAFKA/KIP-500%3A+Replace+ZooKeeper+with+a+Self-Managed+Metadata+Quorum)）。架构简单、操作链路更短、开发成本高。
- **消息的存储**
  - **Topic** 不承担数据存储功能，承担的是逻辑组织的功能，实际的数据存储是在在**分区**维度完成的。
  - 思路：
    - 每个分区单独一个存储文件：Kafka，分区太多会占用系统FD资源，甚至耗尽FD。
    - 一个节点所有分区存储在同一个文件：Pulsar, RocketMQ，无法利用顺序读的优势。
  - 分段存储
    - 如果单个文件过大，数据清理时会很麻烦，效率很低。所以，消息数据都会分段存储。
- **数据清理机制**
  - 实现思路
    - 消费完成执行 ACK 删除数据——无法重放
    - 根据时间和保留大小删除
    - ACK 机制和过期机制相结合——pulsar
  - 删除实现
    - 延时删除、以段数据为单位
...
- **存储性能**
  ...
  - 写入性能：
    ...
    - **缓存写**（先写内存、PageCache）、**批量写**
    - **顺序写**（针对单文件才有效）

  - 写入可靠性

    - 同步刷盘——损失性能
    - WAL预写日志——Pulsar，出现数据丢失时通过WAL来恢复数据，同时可架构随机写转为顺序写；WAL与数据盘分离，WAL需要高性能、容量可以不高
    - 多副本备份
  ...
  - 读取性能
    ...
    - 顺序读 vs. 随机读、批量读
    - 零拷贝
      ![image-20230716152414224](../img/distributed/msg-zero-copy.png)
      - 正常：硬盘 -> ReadBuffer -> 应用程序 -> SocketBuffer -> 网卡设备，四次复制
      - Zero Copy：无需与用户空间应用程序交互。减少数据复制次数、介绍上下文切换次数、通过 DMA 来搬运数据，使数据复制不需要通过 CPU
      - 实现：`mmap`, `sendfile`
...
- 基础功能
  - 连接管理：延迟创建连接？
  - 心跳检测：TCP KeepAlive vs. 应用层主动探测
  - 错误处理：可重试 vs. 不可重试
  - 重试机制
- 生产
  - 客户端寻址机制：元数据寻址 vs. 服务端内部转发（占用服务端资源，影响集群性能）
  - 分区分配策略
  - 批量语义
  - 数据发送方式：同步，异步，发送即忘
...
- 消费模型
  ...
  - **Pull**：客户端不断轮询，根据自身处理速度去拉取数据。缺点是可能出现长时间轮询到空数据的情况，缓解思路：

    - *服务端hold住请求*：如数据为空，则不立即返回，而是等待达到最小数据或达到最大等待时间。
    - *服务端有数据时通知客户端*：立即返回空数据，客户端收到空数据后不再发起请求 而是等待服务端通知。

  - **Push**：可解决消费及时性。三种实现思路：

    - *Broker内置Push*：缺点是当消费者很多时，需要维护很多长链接、并要处理各种客户端异常，消耗服务端资源。
    - *Broker外独立实现Push组件*：本质是先pull再push，开发运维成本高。
    - *客户端伪Push*：客户端内部维护内存队列，底层通过pull拉取数据存储到队列，再回调。——降低用户的使用复杂度。

  - **Pop**：目的是解决客户端实现较重的问题。

    - 客户端不需要感知到分区，直接通过 pop / ack 接口交互。
    - 在服务端管理消费者分区分配、进度管理

    

- 消费模式
  - **独占消费**：例如Kafka，一个分区同一时间只能被一个消费者消费。如果数据倾斜，可能个别分区堆积严重。
  - **共享消费**：分区数据依次投递给不同消费者。无法保证顺序性。
  - **广播消费**：分区数据要投递给所有消费者。实现思路：
    - 内核实现广播消费模型。
    - 使用不同行消费者组。
    - 消费者指定分区消费。
  - **灾备消费**：独占消费的升级版。
...
- 三种节点发现方式
  - **配置文件**：扩容不便
  - **广播机制**：例如通过解析DNS域名，得到所有IP，从而发现所有节点。开发成本高。例子：RabbitMQ、ES
  - **集中式组件**：所有节点都向集中式组件注册和删除自身信息。扩容方便。例子：Kafka zk, RocketMQ, pulsar
- **节点探活**
  - 由 Controller 对集群内所有节点进行探活。
  - 探活方式：主动上报 or 定时探测.
- 主节点选举
  - 依赖第三方组件争抢注册
...
- Follower的职责设计
  - 只负责备份，**不负责读写**：缺点是leader压力较高 --> 可以通过主题分区来缓解
  - 既负责备份，**也负责读取**：缺点是数据一致性不好保证 --> 不好解决
- 副本间数据同步方式
  - 同步复制：主节点收到数据后，通过同步多写的方式发送到从节点。
  - 异步复制：主节点收到数据后，异步发送到从节点，或者从节点异步拉取。
- 一致性考虑
  - **ZooKeeper**：Zab协议实现最终一致性，当多数副本保存数据成功后，就认为这条数据保存成功了。Zab 协议定义了 Zxid、崩溃恢复等细节来保证数据不会丢失
  - **Kafka**：Kafka 的一致性策略是在客户端指定的。客户端会指定 ACK 参数，参数值 -1、0、1，分别表示强一致、弱一致、最终一致。
  - **Pulsar**：配置 Write Quorum Size（Qw）和 Ack Quorum Size（Qa）来控制一致性，Qw 指的是 Ledger 的总副本数，Qa 指数据写入几个副本后算写入成功。
...
- **RabbitMQ**
  - 节点发现：多种支持，固定配置发现、类广播机制发现、第三方组件发现、手动管理。
  - 元数据存储：内置数据库 Mnesia。
  - 副本机制：
    - **镜像队列**：为 Queue 创建副本、完成主从副本之间数据同步、维护主从副本之间数据的强一致性；缺点是性能。（已淘汰）
    - **仲裁队列**：通过 Raft 协议来完成副本间的数据同步和一致性。多数副本写入成功后，就算数据写入成功。
- **RocketMQ**
  - 节点发现：上报 NameServer 中央存储。
  - 元数据存储：元数据实际是存储在 Broker 上，在NameServer上有全量缓存。
  - 部署模式
    - **Master/Slave**：类似MySQL主从原理，Master 负责写入，Slave 负责备份和读取。
      - 副本机制：支持异步复制、同步双写。
      - 问题：主挂了之后从节点你无法成为主，所以创建主题时要在多个主节点创建主题和分区。
    - **DLedger**：核心是通过 Raft 算法实现的 Raft Commitlog，使得 Commitlog 具备了选举和切换的能力。
      - 副本机制：多数原则 + 同步写入，类似ZK。
    - **DLedger Controller**：将 DLedger（Raft）能力进行上移，重新实现了选主组件 DLedger Controller，用来在副本之间进行 Leader 选举和切换。
      - Controller 是一个可选的、松耦合的组件，可以选择内嵌在 NameServer 中，也可以独立部署；也可用在主从模式中。 
      - 副本机制：数据的一致性是可以配置的，可以通过参数 inSyncReplicas 来配置数据写入成功的副本数。
- **Kafka**
  - 节点发现：ZK - `/brokers/ids`，KRaft - ?
  - 副本机制：**ISR 协议**。
    - **副本批量拉取 Leader 数据**：减少网络交互次数，根据ack配置 leader决定等几个follower同步后再返回。
    - **动态维护可用副本集合**：按时间来判断follower落后情况。
    - **控制 Leader 切换和数据截断**：

- **Pulsar**
  - 节点发现：ZK - `/loadbalance/brokers/IP:PORT`
  - 副本机制：Qw + Qa
...
- 优点：计算层无状态可以动态扩缩容、topic可以快速迁移。
  - 存储层：存储结构设计，消息存储格式，数据分段
  - 计算层：协议解析，事务消息，延时消息，等消耗计算资源的模块；
- 存储引擎
  - 对象存储：成本低，但流式读写性能低。
  - 分布式存储：HDFS、BookKeeper 
  - 虚拟云盘：多副本可靠存储；缺点是与broker绑定，分区迁移 leader切换时需要将云盘从老的节点卸载 再挂载到新节点。
- 分区存储模型
  - 每个分区一个文件
- RocketMQ 伪存算分离
  - 5.0 Proxy只是转发层，没有计算和存储逻辑。
  - 后续会将计算逻辑上移到Proxy
- Pulsar存算分离
  - 计算层分区都是单副本，由存储层负责replica
...
- 概念：冷热数据分开存储。
- 优点：降低存储成本。
- 缺点：牺牲性能；牺牲稳定性，第三方存储系统的稳定性肯定会影响消息队列集群的稳定性。
- 流程：
  - 生产消费流程不变；
  - Broker内部模块将本地分段文件上传到远端分布式文件系统中。
  - 消费时如果数据不在本地，就从远端读取返回给客户端。
- 设计：
  - 选择远端文件系统：对象存储服务，或HDFS。
  - 消费**预读远端**：远程的分层文件先下载到本地，消费请求只从本地硬盘读取数据。提前下载好下一份数据分段。缺点无法处理冷启动情况、占用本地硬盘。
  - 消费**流式读取远端**：消费当数据在远程时就流式的从远程存储系统读取数据。缺点是性能无法保证。
  - **隔离性**：上传下载不应影响主流程。
    - CPU：线程绑核，将上传下载线程绑定到某一批固定的CPU核心。
    - 内存：对外内存，DirectIO 精细话控制内存。
    - 网卡：控制同一时间上传和下载的文件数与速度。
  - **回滚**：当远端存储不可用时
    - 暂停上传：新数据段不再上传，都保留在本地；
    - 消费老数据时提示错误。
...
- 定义：无服务器是从客户的角度来理解的，指的是客户不需要关心服务器
- 组件
  - 数据源的触发
    - 事件触发——Q: 如何实现，底层还是拉取？Websocket？
    - 定时触发
    - 流式拉取
  - Serverless运行调度平台
  - 数据目标
- **EDA** 事件驱动架构：主动拉取或被动接收上游不同事件源的数据，然后根据配置好的事件规则，触发执行不同的业务逻辑
  - 事件源格式规范：CloudEvents
  - **接入层**：被动接收（开发支持http cloudevents 等协议的服务器） vs. 主动拉取（binlog, connector）
  - **缓存层**：使用消息队列
  - **运行层**：从缓存层拉取数据，根据规则（或serverless function）进行处理、过滤、聚合，触发调用分发层的接口。
  - **分发层**：对接各种下游系统。
...
- **融合型**：消息（丰富功能、高可靠性、低延时、可追溯） vs. 流（高性能、高吞吐）。
- **多协议**：
- **Serverless**：消息队列是有状态的服务，扩缩容时需要迁移数据，而迁移时间不可控。
- **云原生**：利用云的各种优势特性，让系统更具竞争力
  - **容器化**：快速扩容 - 当拉起新的容器节点后，新节点如何快速承载流量。
  - **虚拟云盘**：在消息队列层移除副本概念，降低复杂度。
  - **对象存储**：降低存储成本。
- **架构简单**：
...
- 配置信息：基于zk，两种思路：
  ...
  - Watch 机制监听 `/config` 下的每个节点，可能有 /config/cluster, brokers, topics,. 
  - 监听一个专门通知配置更新的节点，比如 `/config/notification`。——这种方案允许先变更配置而不立即生效，在需要生效的时候再给 `/config/notification` 写入通知数据，触发配置生效

- 配置信息还可以基于本地文件，结合变更发布系统 更新文件。
...
- 消费进度保存：
  ...
  - **存 zk**：消费者很多时，空间占用大；需要频繁更新zk、zk压力大。
  - **存本地文件**：单机故障时丢失消费进度。——若要分布式，则需要在不同节点间同步文件数据 --> 存到内部压缩主题。
  - **存内部压缩主题**：Kafka
  - **存其他存储引擎**，MySQL、Redis：类似zk思路。
...
- 挑战
  - **多个生产者**：broker接收到的顺序无法保证有序。
  - **异步发送**：异步发送的底层实现是有多个异步线程在负责发送数据，此时可能会失败重试。
  - **多个分区**：跨分区无法保序。

- 实现
  - **单一生产者**：——业务端很难保证吧？
  - **同步发送**：
  - **单一分区**：如果要全局有序则需要，否则可按key分区有序。
- **幂等生产**
  - **思路一：生产send时附带消息ID，服务端通过ID判断是否重复。**
  - 挑战
    - 消息唯一 ID 的生成策略：SnowFlake + 考虑集群ID & 主题ID
    - Broker如何判断ID已被接收过：不能保存所有消息，优化：1）topic维度拆分保存，2）只保存最近消息，3）引入布隆过滤器
  - **思路二：生产者ID + 自增序号seqnum 来标识消息**
  - 优点：
    - 不保留所有的 seqnum，只保留最新收到的 seqnum。
    - 如果收到的消息的 seqnum 是下一条 msg，那么就正常保存数据。否则就放进队列中先等待，等待下一条 msg 收到后，再来判断是否保存该数据；甚至可以直接拒绝消息写入、触发生产者重试。



**3. 延时消息**

- 思路：Broker 在接收到延时消息后，先持久化存储消息，然后标记这个消息**不可见**。再通过内部实现的定时机制，延时到期后将不可见消息变为可见消息，从而让客户端可以正常消费到这条数据。

- **如何让消息可见**
  - **方案一：定时检测写入。**先存储到内置主题，用独立线程去判断数据是否到期，到期则写入实际主题。
    - 对主流程改造小。
    - 延时消息量大时，到期时间会不精准。
  - **方案二：消费时判断数据是否可见。**每次消费时判断是否有到期的延时消息。
    - 流程简单。
  
- **定时机制**
  - 逻辑：随着时间的推进，拿出到期的延时消息进行处理。
  - **方案一：基于轮询检测机制。**独立线程检测临时存储里的数据是否到期。
  - **方案二：基于时间轮机制。**思路大体同上，优点是1）插入获取时间复杂度都是O(1)，2）支持任意时间精度的延时，3）每个时间刻度可以支持任意多元素。
  
- RocketMQ 延时机制
  - 基于轮询检测机制。
  
  - 收到延迟消息后，放入 18 个级别的延时消息队列 `SCHEDULE_TOPIC_XXXX`，每个队列一个线程来定时检测。——不支持任意时间的延迟
  
  - 18个定时任务轮询各队列，判断到期后即把消息发送到原始主题。
  
    > 5.0已支持任意精度，基于时间轮。[TimerMessageStore](https://github.com/apache/rocketmq/blob/develop/store/src/main/java/org/apache/rocketmq/store/timer/TimerMessageStore.java#L724) 
  
- Pulsar 延时机制：消费时检测
  - 延迟投递的消息会先保存到 `Delayed Message Tracker` 数据结构中。Delayed Message Tracker 在堆外内存维护一个 delayed index 优先级队列。
  - 消费时，先去 `Delayed Message Tracker` 检查是否有到期消息。如果有则从 Tracker 中拿出对应的 index，找到对应的消息进行消费。如果没有则直接消费正常的消息。
...
- 定义
  - **生产消息的事务**：RabbitMQ同一批生产的支持原子性。Kafka在同一txnId下支持发送的原子性，commit后才标记为可见。
  - **生产消息+本地事务**：RocketMQ收到半消息并标记为不可见，直到二次确认后标记可见。
  - **消费+处理+生产的事务**：Pulsar 等commit后才标记生产消息可见、并提交消费点位。
- 原理
  - 两阶段提交：生产消息不可见、提交事务标记可见。
- 实现
  - **如何存储事务状态**：内部topic
  - **事务协调者**：统一txn协调者要一致 --> topic partition leader。
  - **如何实现消息可见性**：一是先写入其他存储 提交后再写入实际topic；二是直接在实际topic加入标记。
  - **遇到未提交的事务**：是必须等待，还是先跳过往下消费？都可行，要权衡。
...
- 场景
  - 生产端：写入失败时，临时写入另一个topic，防止生产阻塞。
  - 服务端：数据过期后还没有被消费，写入另一个topic，防止丢失。
  - 消费端：无法处理某条消息时，写入另一个topic，防止消费阻塞。

- 实现：
  - 类似延时消息的思路。 
  - RocketMQ：当一条消息消费失败，RocketMQ 会自动进行重试。达到最大重试次数后，若消费依然失败，则表明消费者在正常情况下无法正确地消费该消息。
...
- 业务自己实现：
  - 每个级别对应一个独立的topic。
- 消息队列内核支持：
  - 思路一：正常写入，同时维护一个按照优先级排序后的消息索引；消费时根据索引顺序去定位读取。
    - 数据结构：排序链表、归并排序
    - 索引持久化：无需，重启后读取分区 重建数据。
  - 思路二：写入时对消息全量重排序；正常消费。——不可行。
  - 思路三：空间换时间，只支持固定维度优先级；写入时按优先级写入不同文件段。消费时优先消费高优先级。
...
- 目的：Push模式，Broker 收到 消息后，可以直接将消息推送给客户端 
...
- 目的：跟踪单条消息的运行周期 
- 设计
  - **消息ID**：UUID，SnowFlake，自增ID
  - **记录**：旁路，不能影响主流程的稳定性和性能；
    - 方案一：记录到本地文件，然后采集上报到存储引擎（Fluend --> ES）；
    - 方案二：记录到内置主题，第三方系统消费该主题、写入持久存储引擎；（推荐）
    - 方案三：直接写入第三方存储，例如ES；
    - 方案四：单机维度存储引擎，例如写入单机 RocksDB；查询时遍历各个机器
  - **存储**：写多读少，写入性能要优秀；数据不能丢、能长期保留、具备检索能力。
    - 因为内容会放大，所以要尽量精简（ID+时间）。
    - ES：检索能力强、成本高；
    - *ClickHouse*：
    - HBase：
  - **查询**：最小可能的多维度查询。
...
- **冲突检测**
  - 同步检测：等待写入被复制到多个副本，发现冲突则失败。相当于退化为单主节点复制。
  - 异步检测：更常用
- **冲突避免**
  - 更理想的情况是避免冲突发生。例如特定用户总是路由到同一个 DC、例如不同分区的主副本放在不同DC。
- **冲突收敛**：实在无法避免冲突时使用
  ...
  - **应用层解决冲突**：使用合适的数据结构记录冲突相关的所有信息，由应用代码稍后解决冲突；
    - 写入时解决冲突
      - 存储系统在复制变更日志时检测到冲突，即调用应用代码*回调*解决冲突。
      - 例如 Bucardo
    - 读取时解决冲突
...
![multi-leader topologies](../img/distributed/replication-topo.png)
...
- w 和 r 通常取 majority of nodes，超过半数
...
- **Sloppy Quorum**：w 或 r 无法达到要求时如何处理？
  - 拒绝请求
  - 或接受写入，写入少数节点  --> `Sloppy Quorum`；一旦其他节点恢复，立即回传到这些节点 --> `Hinted Handoff`
  
    > 接受写入，并且将新的写入暂时交给一些正常节点? 
...
- **按 Key Range 分区**
  ...
  > 为每个分区分配一段连续的 key range。例如百科全书按字母序分册。
...
- 二级索引
  - 用于搜索；
  - 如何对多个分区的数据构建索引？
- **本地索引，按文档** - Partitioning Secondary Indexes **by Document**
...
- **全局索引，按词条** - Partitioning Secondary Indexes **by Term**
...
- **策略一：静态分区，固定分区数**
  ...
  - 前提：提前确定好分区数。推荐让分区数大于节点数；大分区 --> 难恢复；小分区 --> 开销大 (元信息、均衡调度)。
...
![routing request ways](../img/distributed/partition-router.png)
...
|                   | 更新缓存                                                     | 特点                                            | 一致性             |
| ----------------- | ------------------------------------------------------------ | ----------------------------------------------- | ------------------ |
| **Write Back**    | 直接更新缓存值。<br />TTL到期之后清除缓存值，并将更新后的值写入 DB。 | Cache is Source of Truth；Efficiency 但持久性低 | 最终一致           |
| **Write Through** | Evict Cache，同时更新 DB。                                   | DB is Source of Truth                           | 强一致             |
| **Wirte Around**  | 只更新 DB。Cache 根据TTL过期。                               | DB is Source of Truth；Efficiency 且持久性高    | 最终一致，持久性高 |
...
- 思想
  - 写入数据时，只写缓存，并把缓存块标记为脏；而由 cache 负责更新 DB。
    ——**缓存值是 Source of Truth，DB 最终一致性。**
  - 脏块只有被再次使用时才会将其中的数据写入到后端存储 -- 或者 TTL 到期

- 何时写回 DB:
  - **Timeout-based Persistence**：TTL 到期后被 evict 并写回 DB；
  - **Event-based Write Back**：缓存元素被更新 N 次之后即写回 DB；
  - **Replacement Write Back**：每个缓存元素有一个attr，表示是否被更新过；evict 时根据该标志决定是否更新 DB。
- 特点
  - 效率高
  - 最终一致
- 问题
  - 如果缓存宕机，可能有部分数据没有写入DB？
...
- 写
  - 若缓存中不存在，则 Write Allocate：写入缓存，再由缓存组件同步更新到数据库
  - 若缓存已存在
    - update DB，再由 DB push to cache
    - 或者 evict cache，then update DB. （非同步更新）
...
- 思想：用户只与缓存打交道（x）
  - 同时 **evict** cache + **update** main memory
- 特点
  - 高持久性
  - 强一致性
...
- 案例
  - 例如 Guava Cache
- 问题：
  - 不适合本地缓存，因为缓存更新时 可能只更新了一个实例上的本地缓存。
...
- **一致性 Consistency**
  ...
  - 弱一致性
  - 最终一致性：
    - 例一，发送邮件已成功，但状态还是发送中，不过最终会移到发件箱
    - 例二，Quorum
  - 因果一致性：对同一个 key 保证顺序（部分强一致性）
- **持久性 Durability**
  - 数据复制
