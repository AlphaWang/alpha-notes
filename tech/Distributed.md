...
- 并行事务的 Race Condition 问题
  ...
  - **脏写**：覆盖了另一事务尚未提交的写入。——所有隔离级别均能解决脏写

  - **不可重复读 / Read skew**：对同一行数据的两次查询结果不同。——快照隔离解决

    > 因为缺乏贯穿事务周期的`读锁`，数据可能被另一事务修改。

  - **更新丢失**：多个事务进行 read-modify-write，一个事务覆盖了另一个的写入。——select for update

  - **写倾斜 / Write skew**：当写入时，之前读取到的前提已过时。——串行化

  - **幻读**：一个事务改变了另一个事务的查询结果。（以致第二次读可能读到新插入的记录？）
...
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 特点 | 写入更快；                                                   | 读取更快；                                                   |
| 更新 | Out-of-place update：异位更新，将更新的内容存储到新位置，而不是覆盖旧条目；需要 compaction。更适用于固态硬盘。 | In-place update：就地更新。适用于机械硬盘。                  |
