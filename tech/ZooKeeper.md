...
- **服务端**
  ...
  - 服务端存储ServerCnxn
    ...
    - 客户端并不会将 Wathcer 对象真正传到服务端，否则服务端内存紧张

  - 服务端触发通知

    - 1. 封装 WatchedEvent。注意不包含被更新的数据！！！，需要推拉结合
    ...
    - 2. 查询并删除Watcher
      3. process: send response (header = -1)：通过 ServerCnxn 将 Event 对象传递给客户端

- **客户端**执行回调

  - 1.SendThread 接收通知， 放入EventThread （NIO）
  - 2.查询并删除Watcher
  - 3.process: 执行回调
